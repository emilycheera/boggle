    ...
    for i in len(board):
        for j in len(board[i]):
            if word[word_idx] == board[i][j]:
                word_idx += 1
                word_location.append(board[i][j])
                #CALL NEWS(word_location[-1], word_idx)
                #if true call news again
                #if false word_location.pop()
                    #word_idx -= 1
                    #go back into for j loop


def check_NEWS(location, idx):
